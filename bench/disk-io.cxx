...
int main(int argc, char **argv) {
  ...
  for (size_t i(0); i < elements_per_rank; i++) wdata[i] = 3.14/(i+2.1);
  ...
  MPI_File file;
  MPI_File_open(MPI_COMM_WORLD, "f.tmp",  MPI_MODE_RDWR | MPI_MODE_CREATE, MPI_INFO_NULL, &file);
  ...
  MPI_File_write_at(file, file_offset, wdata, elements_per_rank, mpiType, &stat);
  MPI_Barrier(MPI_COMM_WORLD);
  double write_time = MPI_Wtime() - start_write;

  if (!rank) printf("Write file: %lf s, bandwidth: %lf GB/s\n", write_time, total_elements*to_GB / write_time);
  ...
  if (!rank)
    printf("Read file naively: %lf s, bandwidth: %lf GB/s, 1 read %ld elements\n",
           naive_read_time, total_elements*to_GB / naive_read_time, elements_per_rank);
  ...
  for (size_t i(0); i < buffer_per_rank; i++) {
    for (size_t k(0); k < nk*nk; k++) {
      ...
      MPI_File_read_at(file, file_offset, rdata + array_offset, no*no*nv, mpiType, &stat);

      MPI_Barrier(MPI_COMM_WORLD);
      double round_robin_read_time = MPI_Wtime() - start_round_robin_read;
      total_round_robin_read_time += round_robin_read_time;
      //if (!rank) printf("Time to read the file in round robin style: %lf, overall bandwidth: %lf\n", round_robin_read_time, buffer_size*np*to_GB / round_robin_read_time);
    }
  }
  if (!rank)
    printf("Read file round robin style: %lf s, bandwidth: %lf GB/s, %ld reads %ld elements\n",
           total_round_robin_read_time, total_elements*to_GB / total_round_robin_read_time, nk*nk, no*no*nv);
