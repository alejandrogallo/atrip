...
std::vector<std::string> getNodeNames(MPI_Comm comm) {
  ...
  std::vector<int> nameLengths(np), off(np);
  int nameLength;
  MPI_Get_processor_name(nodeName, &nameLength);
  MPI_Allgather(&nameLength, 1, MPI_INT, nameLengths.data(), 1, MPI_INT, comm);
...
std::vector<RankInfo> getNodeInfos(std::vector<string> const &nodeNames) {
  ...
  for (auto const &name : nodeNames) {
    if (uniqueNames.size() == 0
        || uniqueNames.end()
               == std::find(uniqueNames.begin(), uniqueNames.end(), name)) {
  ...
  for (auto const &name : nodeNames) {
    ...
    result.push_back(
        {name,
         ...
         globalRank++,
         localRanks[nodeId]++,
         (size_t)std::count(nodeNames.begin(), nodeNames.end(), name)});
  }
  return result;
}
...
inline size_t isOnNode(size_t tuple, size_t nNodes) { return tuple % nNodes; }

std::vector<size_t> getTupleNodes(ABCTuple const &t, size_t nNodes) {
  std::vector<size_t> nTuple = {isOnNode(t[0], nNodes),
                                isOnNode(t[1], nNodes),
                                isOnNode(t[2], nNodes)};
...
ABCTuples specialDistribution(Info const &info, ABCTuples const &allTuples) {
  ...
  std::vector<ABCTuples> container1d(nNodes), container2d(nNodes * nNodes),
      container3d(nNodes * nNodes * nNodes);

  WITH_DBG if (info.nodeId == 0) std::cout
      << "\tGoing through all " << allTuples.size() << " tuples in " << nNodes
      << " nodes\n";
  ...
  for (auto const &t : allTuples) {
    ...
    case 2: container2d[_nodes[0] + _nodes[1] * nNodes].push_back(t); break;
    case 3:
      container3d[_nodes[0] + _nodes[1] * nNodes + _nodes[2] * nNodes * nNodes]
          .push_back(t);
      break;
    }
  }

  WITH_DBG if (info.nodeId == 0) std::cout << "\tBuilding 1-d containers\n";
  ...
  WITH_DBG if (info.nodeId == 0) std::cout << "\tBuilding 2-d containers\n";
  ...
  for (size_t yx = 0; yx < container2d.size(); yx++) {
    ...
    const size_t idx = yx % nNodes
        // remeber: yx = idy * nNodes + idx
        ,
                 idy = yx / nNodes, n_half = _tuples.size() / 2,
  ...
  for (size_t zyx = 0; zyx < container3d.size(); zyx++) {
    ...
    const size_t idx = zyx % nNodes,
                 idy = (zyx / nNodes) % nNodes
        // remember: zyx = idx + idy * nNodes + idz * nNodes^2
        ,
                 idz = zyx / nNodes / nNodes, n_third = _tuples.size() / 3,
                 size = nodeTuples.size();
  ...
  for (auto &nt : nodeTuples) {
    if (isOnNode(nt[0], nNodes) == info.nodeId) {   // 1234
      ...
      if (isOnNode(nt[1], nNodes) == info.nodeId       // 56
          && isOnNode(nt[2], nNodes) != info.nodeId) { // 6
        size_t const x(nt[1]);
...
#if ATRIP_DEBUG > 1
  WITH_DBG if (info.nodeId == 0) std::cout << "checking for validity of "
                                           << nodeTuples.size() << std::endl;
  const bool anyInvalid =
      std::any_of(nodeTuples.begin(), nodeTuples.end(), [](ABCTuple const &t) {
        return t == INVALID_TUPLE;
      });
...
std::vector<ABCTuple> main(MPI_Comm universe, size_t Nv) {
  ...
  // element per node
  bool const computeDistribution = nodeInfos[rank].localRank == 0;
  ...
  const int color = nodeInfos[rank].nodeId, key = nodeInfos[rank].localRank;
  ...
  size_t const tuplesPerRankLocal =
  ...
  MPI_Bcast(&tuplesPerRankGlobal, 1, MPI_UINT64_T, 0, universe);
  ...
  size_t const totalTuples = tuplesPerRankGlobal * nodeInfos[rank].ranksPerNode;
